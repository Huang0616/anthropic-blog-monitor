...
服务使用 **时间戳对比 + 数据库记录** 的方式判断新文章：

1. **记录最后检查时间**：数据库 `scraper_state` 表存储上次成功检查的时间
2. **提取文章发布时间**：抓取文章时尝试从内容中提取发布日期
3. **对比时间戳**：只处理发布时间晚于最后检查时间的文章
4. **更新检查时间**：成功处理后更新最后检查时间

**优势**：
- 不会重复推送已处理的文章
- 即使服务重启也能正确判断新文章
- 支持补抓历史文章（修改 `last_check` 时间即可）
