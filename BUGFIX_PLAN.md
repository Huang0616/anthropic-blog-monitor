...
| 编号 | Bug | 现象 | 根本原因 | 优先级 |
|------|-----|------|----------|--------|
| **BUG-001** | 文章内容提取失败 | 数据库中 `content` 字段为空 | 爬虫正则表达式无法正确解析 Next.js SSR 页面 | 🔴 P0 |
| **BUG-002** | 摘要生成失败 | `summary` 字段为空 | 依赖 content 字段，但 content 为空 | 🔴 P0 |
| **BUG-003** | 飞书推送未生效 | 收不到新文章通知 | 未配置 `.env` 文件，webhook_url 为空 | 🔴 P0 |
...
| 日期 | 任务 | 负责人 | 状态 |
|------|------|--------|------|
| 2026-02-24 | BUG-001: 内容提取修复 | 大尾巴猫 | ⏳ 待执行 |
| 2026-02-24 | BUG-002: 摘要降级策略 | 大尾巴猫 | ⏳ 待执行 |
| 2026-02-24 | BUG-003: 飞书配置 | 老大 | ⏳ 待提供 Webhook |
| 2026-02-25 | BUG-004: 标题提取优化 | 大尾巴猫 | ⏳ 待执行 |
| 2026-02-25 | BUG-005: 日期提取 | 大尾巴猫 | ⏳ 待执行 |

**验收标准**:
- [ ] 手动触发 `/scrape` 后，数据库中有 content 和 summary
- [ ] 飞书能收到新文章推送
- [ ] 前端能正常显示文章列表
